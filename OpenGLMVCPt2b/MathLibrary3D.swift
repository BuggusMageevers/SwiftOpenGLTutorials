...
struct Matrix4: CustomStringConvertible, Hashable {
    ...
    /**
     OpenGL expects an inverted order from how we have set up our matrix.
     Our matrix is row-major while OpenGL expects column major. This
     translates the current matrix order.
     - returns:
        a column major `array<Float>` of the current Matrix4.
     */
    func asArray() -> [Float] {
